"""
https://jhuisi.github.io/charm/charm/schemes/abenc/abenc_maabe_rw15.html
"""
...
if __name__ == '__main__':
    ...
    gid = "patient1"
    user_attributes1 = ['PATIENT@HOSPITAL1', 'PATIENT@HOSPITAL2']
    user_attributes2 = ['EMPLOYEE@HOSPITAL1', 'EMPLOYEE@HOSPITAL2']
    user_keys1 = maabe.multiple_attributes_keygen(public_parameters, secret_key1, gid, user_attributes1)
    user_keys2 = maabe.multiple_attributes_keygen(public_parameters, secret_key2, gid, user_attributes2)
    user_keys = {'GID': gid, 'keys': merge_dicts(user_keys1, user_keys2)}
    ...
    # We can only encrypt group elements with ABE, therefore we first need to encrypt using symmetric encryption
    pt = b'Record element'
    pt_random_group_element = group.random(GT)
    a = SymmetricCryptoAbstraction(extract_key(pt_random_group_element))
    ct = a.encrypt(pt)
    print('Message to be encrypted: {}'.format(pt))

    # Encrypt message using random group element as key
    access_policy = '(PATIENT@HOSPITAL1 or PATIENT@HOSPITAL2 or' \
                    ' EMPLOYEE@HOSPITAL1 or EMPLOYEE@HOSPITAL2)'
    ct_random_group_element = maabe.encrypt(public_parameters, public_keys, pt_random_group_element, access_policy)

    # Compare plain text with decrypted
    decrypted_random_group_element_message = maabe.decrypt(public_parameters, user_keys, ct_random_group_element)
    print('Random group elements are equal: {}'.format(decrypted_random_group_element_message == pt_random_group_element))
    print('Symmetric encryption still equal: {}'.format(pt == a.decrypt(ct)))
